...
function DemoForm() {
  ...
  const inputCls = "w-full px-4 py-3 rounded-md bg-white border border-gray-200 text-gray-900 placeholder-gray-400 text-sm font-sans focus:outline-none focus:border-green-400 focus:ring-2 focus:ring-green-400/10 transition-all duration-200";
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        ...
        <select name="members" value={form.members} onChange={handleChange}
          className="w-full px-4 py-3 rounded-md bg-white border border-gray-200 text-gray-700 text-sm font-sans focus:outline-none focus:border-green-400 focus:ring-2 focus:ring-green-400/10 transition-all duration-200">
      ...
      {error && <p className="text-red-500 text-xs flex items-center gap-1.5 font-mono"><AlertCircle size={12} /> {error}</p>}
      <button type="submit"
        className="group w-full inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-md bg-neon-green text-dark-950 font-heading font-medium text-sm hover:opacity-90 transition-all duration-300">
        <MessageCircle size={15} /> SEND ON WHATSAPP
      </button>
      <p className="text-center text-gray-400 text-[11px] font-mono">Opens WhatsApp with your details pre-filled</p>
    </form>
  );
}
...
function FAQ() {
  ...
  return (
    <div className="space-y-2">
      {faqs.map((f, i) => (
        <div key={i} className="rounded-lg bg-white border border-gray-200 shadow-sm overflow-hidden transition-all duration-300">
          <button onClick={() => setOpen(open === i ? null : i)}
            className="w-full flex items-center justify-between px-6 py-5 text-left gap-4 hover:bg-gray-50 transition-colors">
            <span className="font-heading font-medium text-gray-900 text-sm sm:text-base leading-snug">{f.q}</span>
            <ChevronDown size={16} className={`text-green-500 shrink-0 transition-transform duration-300 ${open === i ? "rotate-180" : ""}`} />
          </button>
          {open === i && (
            <div className="px-6 pb-5 border-t border-gray-100">
              <p className="text-gray-600 text-sm leading-relaxed font-sans pt-4">{f.a}</p>
            </div>
          )}
        </div>
      ))}
...
function StickyBar({ onDemo }) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex border-t border-gray-200 bg-white/95 backdrop-blur-md shadow-lg">
      <a href="tel:+918147814232"
        className="flex-1 flex flex-col items-center justify-center gap-1 py-3 hover:bg-gray-50 transition-colors border-r border-gray-100">
        <Phone size={17} className="text-gray-500" />
        <span className="text-[10px] font-mono uppercase tracking-wider text-gray-400">Call Us</span>
      </a>
      <a href={`https://wa.me/918147814232`} target="_blank" rel="noreferrer"
        className="flex-1 flex flex-col items-center justify-center gap-1 py-3 bg-[#25D366] hover:bg-[#20bd5a] transition-colors border-r border-gray-100">
        <MessageCircle size={17} className="text-white" />
        <span className="text-[10px] font-mono uppercase tracking-wider text-white font-bold">WhatsApp</span>
      </a>
      <button onClick={onDemo}
        className="flex-1 flex flex-col items-center justify-center gap-1 py-3 bg-neon-green hover:opacity-90 transition-colors">
...
export default function GymLanding() {
  ...
  return (
    <div className="overflow-hidden relative pb-[60px] md:pb-0 bg-white">
      <StickyBar onDemo={scrollToForm} />

      {/* ─── NAV ─── */}
      <nav className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-4 sm:px-6 lg:px-8 py-4 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
        <span className="font-heading text-lg font-medium flex items-center gap-2">
          <Dumbbell size={19} className="text-neon-green" />
          <span className="text-gray-900">GymOS</span>
          <span className="text-gray-400 text-xs font-mono hidden sm:inline">by Devloft</span>
        </span>
        <div className="flex items-center gap-4">
          <a href={`tel:${PHONE_NUM}`} className="hidden md:flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors duration-200">
            <Phone size={13} className="text-neon-green" /> {PHONE_NUM}
          </a>
          <a href={`https://wa.me/${WHATSAPP_NUM}`} target="_blank" rel="noreferrer"
            className="hidden sm:flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors duration-200">
      ...
      {/* ─── HERO ─── */}
      <section className="relative min-h-screen flex items-center pt-20 pb-12 bg-white">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-neon-green/[0.06] rounded-full blur-[130px] pointer-events-none" />
        <div className="absolute bottom-1/3 right-1/4 w-[400px] h-[400px] bg-electric-blue/[0.07] rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-7">
            <motion.p variants={fadeUp} className="text-sm font-mono uppercase tracking-widest text-green-500">
            ...
            <motion.h1 variants={fadeUp} className="font-heading text-5xl sm:text-6xl lg:text-[4.2rem] font-medium tracking-tight leading-[1.08]">
              <span className="text-gray-900">// Gym ERP</span>
              <br />
              <span className="gradient-text">built for India</span>
            </motion.h1>
            <motion.p variants={fadeUp} className="text-gray-600 text-lg leading-relaxed max-w-lg">
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-600 max-w-md">
              {[
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
              ...
              <a href={`https://wa.me/${WHATSAPP_NUM}`} target="_blank" rel="noreferrer"
                className="group inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full bg-white border border-gray-200 text-gray-800 font-heading font-medium hover:bg-gray-50 hover:border-green-300 transition-all duration-300 shadow-sm">
                <MessageCircle size={16} className="text-neon-green" /> WhatsApp Us
              </a>
            </motion.div>
            <motion.p variants={fadeUp} className="text-gray-400 text-xs font-mono">
          ...
          <motion.div ref={formRef} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            className="rounded-xl bg-white border border-gray-200 shadow-xl shadow-gray-100/80 p-8">
            <p className="text-sm font-mono uppercase tracking-widest text-green-500 mb-1">Book a demo</p>
            <h2 className="font-heading text-2xl font-medium text-gray-900 mb-5">See everything live in 15 mins</h2>
            <DemoForm />
          </motion.div>
        </div>
      </section>
      ...
      <Section className="py-10 bg-gray-50 border-y border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.p variants={fadeUp} className="text-center text-xs font-mono uppercase tracking-widest text-gray-400 mb-5">
            Trusted by gyms across India
          </motion.p>
          <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-x-10 gap-y-3">
            {clientLogos.map((logo) => (
              <span key={logo} className="text-gray-400 text-sm font-heading font-medium hover:text-gray-700 transition-colors duration-200">{logo}</span>
            ))}
          </motion.div>
        </div>
      </Section>
      ...
      {/* ─── MODULES ─── */}
      <Section className="py-28 relative bg-gray-50">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-electric-blue/[0.04] rounded-full blur-[120px] pointer-events-none" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-16">
            <motion.div variants={fadeUp}>
              <p className="text-sm font-mono uppercase tracking-widest text-green-500 mb-4">What's inside</p>
              <h2 className="font-heading text-4xl sm:text-5xl font-light text-gray-900 leading-tight">
                Ten modules.
                <br />
                <span className="gradient-text-alt">One complete ERP.</span>
              </h2>
            </motion.div>
            <motion.p variants={fadeUp} custom={1} className="text-gray-600 text-lg leading-relaxed max-w-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {modules.map(({ icon: Icon, tag, title, desc, bullets, accent }, i) => (
              <motion.div key={title} variants={fadeUp} custom={i % 3}>
                <div className="group rounded-lg bg-white border border-gray-200 shadow-sm hover:shadow-md hover:border-green-300/50 p-7 h-full transition-all duration-300">
                  <div className="flex items-start justify-between mb-5">
                    <Icon size={22} className={accent} />
                    <span className="font-mono text-[9px] uppercase tracking-widest text-gray-400">{tag}</span>
                  </div>
                  <h3 className="font-heading text-lg font-medium text-gray-900 mb-2">{title}</h3>
      ...
      <Section className="py-28 bg-white relative">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16">
            <p className="text-sm font-mono uppercase tracking-widest text-cyan-500 mb-4">Simple onboarding</p>
            <h2 className="font-heading text-4xl sm:text-5xl font-light text-gray-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {steps.map((step, i) => (
              <motion.div key={step.num} variants={fadeUp} custom={i}>
                <div className="group rounded-lg bg-white border border-gray-200 shadow-sm hover:shadow-md p-8 h-full transition-all duration-300">
                  <span className="font-mono text-4xl font-light text-green-100 block mb-6">{step.num}</span>
                  <h3 className="font-heading text-xl font-medium text-gray-900 mb-3">{step.title}</h3>
      ...
      {/* ─── PRICING ─── */}
      <Section className="py-28 relative bg-gray-50">
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-[300px] bg-neon-green/[0.04] rounded-full blur-[100px] pointer-events-none" />
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div variants={fadeUp} className="text-center mb-16 max-w-2xl mx-auto">
            <p className="text-sm font-mono uppercase tracking-widest text-blue-500 mb-4">Transparent Pricing</p>
            <h2 className="font-heading text-4xl sm:text-5xl font-light text-gray-900 mb-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
              ...
              {plans.map((plan, i) => (
                <motion.div key={plan.name} variants={fadeUp} custom={i}
                  className={`relative rounded-xl p-8 transition-all duration-300 ${plan.popular
                    ? "border-2 border-green-400/50 bg-white shadow-lg shadow-green-50"
                  ...
                  <div className="flex items-start justify-between gap-4 mb-5">
                    <div>
                      <h3 className="font-heading text-xl font-medium text-gray-900 mb-1">{plan.name}</h3>
                      <p className="text-gray-500 text-xs font-mono">{plan.tagline}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-heading text-3xl font-light text-gray-900 leading-none">
                        {plan.price}
                        <span className="text-sm text-gray-400 font-mono ml-1">{plan.period}</span>
                      </p>
                      {plan.saving && <p className="text-green-600 text-xs font-mono mt-1">{plan.saving}</p>}
                    </div>
                  </div>
                  <button onClick={scrollToForm}
                    className={`group w-full inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-md font-heading font-medium text-sm transition-all duration-300 ${plan.popular
                      ? "bg-neon-green text-dark-950 hover:opacity-90"
            ...
            <motion.div variants={fadeUp} custom={1} className="rounded-xl bg-white border border-gray-200 shadow-sm p-8">
              <p className="text-xs font-mono uppercase tracking-widest text-green-600 mb-5">Everything included in both plans</p>
              <ul className="space-y-2.5">
                {allFeatures.map((f) => (
                  <li key={f} className="flex items-center gap-3 text-gray-600 text-sm font-sans">
      ...
      <Section className="py-28 bg-white relative">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-16">
            <motion.div variants={fadeUp}>
              <p className="text-sm font-mono uppercase tracking-widest text-green-500 mb-4">Real gyms, real results</p>
              <h2 className="font-heading text-4xl sm:text-5xl font-light text-gray-900 leading-tight">
                500+ gyms.
                <br />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {testimonials.map(({ name, role, city, members, rating, text }, i) => (
              <motion.div key={name} variants={fadeUp} custom={i}>
                <div className="group rounded-lg bg-white border border-gray-200 shadow-sm hover:shadow-md p-7 h-full transition-all duration-300">
                  <div className="flex gap-1 mb-4">
                    {Array.from({ length: rating }).map((_, j) => <Star key={j} size={13} className="text-neon-green" fill="#00ff88" />)}
                  </div>
                  <p className="text-gray-600 text-sm leading-relaxed mb-6 font-sans">"{text}"</p>
                  <div className="border-t border-gray-100 pt-5">
                    <p className="font-heading font-medium text-gray-900 text-sm">{name}</p>
                    <p className="text-gray-400 text-xs font-mono mt-0.5">{role} · {city}</p>
                    <span className="inline-flex items-center gap-1 mt-2 text-[10px] font-mono text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">
      ...
      <Section className="py-16 bg-gray-50 border-y border-gray-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-2.5">
            {cities.map((city) => (
              <span key={city}
                className="text-sm text-gray-500 bg-white border border-gray-200 px-4 py-1.5 rounded-full font-sans hover:border-green-300 hover:text-gray-800 transition-all duration-200 cursor-default shadow-sm">
                {city}
              </span>
            ))}
          </motion.div>
          <motion.p variants={fadeUp} className="text-gray-400 text-xs font-mono mt-6">
      ...
      <Section className="py-28 bg-white relative">
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div variants={fadeUp} className="text-center mb-14">
            <p className="text-sm font-mono uppercase tracking-widest text-blue-500 mb-4">FAQs</p>
            <h2 className="font-heading text-4xl sm:text-5xl font-light text-gray-900">
          ...
          <motion.div variants={fadeUp}
            className="mt-10 rounded-xl bg-gray-50 border border-gray-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-5">
            <div>
              <p className="font-heading font-medium text-gray-900 mb-1">Still have questions?</p>
              <p className="text-gray-500 text-sm font-sans">Talk to our team — we're here to help.</p>
            </div>
            <div className="flex gap-3 shrink-0">
              <a href={`https://wa.me/${WHATSAPP_NUM}`} target="_blank" rel="noreferrer"
                className="group inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-neon-green text-dark-950 font-heading font-medium text-sm hover:opacity-90 transition-all duration-300">
                <MessageCircle size={14} /> WhatsApp Us
              </a>
              <a href={`tel:${PHONE_NUM}`}
                className="group inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white border border-gray-200 text-gray-800 font-heading font-medium text-sm hover:bg-gray-50 hover:border-gray-300 transition-all duration-300 shadow-sm">
      ...
      {/* ─── FOOTER ─── */}
      <footer className="bg-gray-50 border-t border-gray-200 py-14 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <p className="text-gray-500 text-sm font-sans leading-relaxed mb-4">
              Complete gym management ERP built for Indian gym owners.
            </p>
            <p className="text-gray-400 text-xs font-mono">by Devloft Technologies, Bangalore</p>
          </div>
          <div>
            <p className="font-heading font-medium text-gray-900 mb-4 text-sm">Modules</p>
            <ul className="space-y-2 text-sm text-gray-500 font-sans">
              {["Member Management", "Payments & Billing", "QR Attendance", "Class Scheduling", "Staff Management", "Analytics Dashboard"].map((s) => (
                <li key={s} className="hover:text-gray-800 transition-colors duration-200 cursor-default">{s}</li>
              ))}
            </ul>
          </div>
          <div>
            <p className="font-heading font-medium text-gray-900 mb-4 text-sm">Top Cities</p>
            <ul className="space-y-2 text-sm text-gray-500 font-sans">
              {["Bangalore", "Mumbai", "Hyderabad", "Chennai", "Delhi NCR", "Pune"].map((c) => (
                <li key={c} className="hover:text-gray-800 transition-colors duration-200 cursor-default">Gym Software {c}</li>
              ))}
            </ul>
          </div>
          <div>
            <p className="font-heading font-medium text-gray-900 mb-4 text-sm">Contact</p>
            <ul className="space-y-3 text-sm text-gray-500 font-sans">
              <li><a href={`tel:${PHONE_NUM}`} className="flex items-center gap-2 hover:text-gray-800 transition-colors duration-200"><Phone size={13} className="text-neon-green" /> {PHONE_NUM}</a></li>
              <li><a href={`https://wa.me/${WHATSAPP_NUM}`} className="flex items-center gap-2 hover:text-gray-800 transition-colors duration-200"><MessageCircle size={13} className="text-electric-cyan" /> WhatsApp Us</a></li>
        ...
        <div className="max-w-7xl mx-auto mt-12 pt-6 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-gray-400 text-xs font-mono">
